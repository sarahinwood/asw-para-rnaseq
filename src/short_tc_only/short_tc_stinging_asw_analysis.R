...
dds <- dds[,-1]
##Save dds
saveRDS(dds, file = "output/asw_timecourse/short_tc_deseq2/dds.rds")

##read in dds to save rerunning
dds <- readRDS("output/asw_timecourse/short_tc_deseq2/dds.rds")
counts_matrix <- data.frame(counts(dds))
counts_colSums <- data.frame(colSums(counts_matrix, na.rm=TRUE))
fwrite(counts_colSums, "output/asw_timecourse/short_tc_deseq2/counts_colsums.csv", row.names = TRUE)
...
saveRDS(dds_abdo, file = "output/asw_timecourse/short_tc_deseq2/dds_abdo.rds")

dds_abdo <- readRDS("output/asw_timecourse/short_tc_deseq2/dds_abdo.rds")
...
fwrite(data.table(sig_gene_names), "output/asw_timecourse/short_tc_deseq2/timecourse_sig_gene_names.csv")
...
fwrite(timecourse_all, "output/asw_timecourse/short_tc_deseq2/timecourse_all_genes.csv")
...
fwrite(ordered_degs_table, "output/asw_timecourse/short_tc_deseq2/timecourse_analysis_sig_degs.csv")
...
fwrite(sig_w_annots, "output/asw_timecourse/short_tc_deseq2/sig_genes_with_annots.csv")

##sort out DEGs that are new and DEGs that have been lost and take a look at annots

##read back in dedeup degs with annots
dedup_sig_w_annots <- fread("output/asw_timecourse/short_tc_deseq2/dedup_sig_genes_with_annots.csv")
...
fwrite(list_degs_no_annot, "output/asw_timecourse/short_tc_deseq2/no_annot/degs_with_no_annot.txt")
...
fwrite(cell_imm_rnaseq_res, "output/asw_timecourse/short_tc_deseq2/cellular_immune_deseq_res.csv")
