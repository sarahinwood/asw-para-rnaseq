...
quant_files <- list.files(path="output/asw", pattern = "quant.sf", full.names=TRUE, recursive = TRUE)
...
fwrite(ordered_sig_res_group_table, "output/deseq2/exposed/exposed_analysis_sig_degs.csv", col.names = TRUE, row.names = FALSE)
...
fwrite(sig_w_annots, "output/deseq2/exposed/sig_genes_with_annots.csv")

  ##read back in dedeup sig genes w/annots
dedup_sig_w_annots <- fread("output/deseq2/exposed/dedup_sig_genes_with_annots.csv")
...
fwrite(list_degs_no_annot, "output/deseq2/exposed/degs_with_no_annot.txt")
